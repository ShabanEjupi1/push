...
public class ExcelExporterService {
    ...
    public void exportInventoryEXLSX(String whsCod, Date dateFrom, Date dateTo, List<String> headers, OutputStream outputStream) throws Exception {
        ...
        String sql;
        if (filterByWhs) {
            sql = "SELECT REGIME, IDE_CUO_COD, OFFICE_NAME, IN_REG_NBR, IDE_REG_DAT, WHS_TIM, DEC_COD, DEC_REF_YER, CMP_CON_COD, WHS_COD, KEY_ITM_NBR, TAR_PRC_EXT, TARIK, VIT_WGT_NETI, VIT_WGT_NET, VIT_STV, GDS_ORG_CTY, QTY_I, QTY_R, TAR_SUP_COD, GDS_DS3, TAR_PRF FROM WHS_SAD_INVENTORY_E WHERE WHS_COD = ? AND IDE_REG_DAT BETWEEN ? AND ? ORDER BY IDE_REG_DAT";
        } else {
            sql = "SELECT REGIME, IDE_CUO_COD, OFFICE_NAME, IN_REG_NBR, IDE_REG_DAT, WHS_TIM, DEC_COD, DEC_REF_YER, CMP_CON_COD, WHS_COD, KEY_ITM_NBR, TAR_PRC_EXT, TARIK, VIT_WGT_NETI, VIT_WGT_NET, VIT_STV, GDS_ORG_CTY, QTY_I, QTY_R, TAR_SUP_COD, GDS_DS3, TAR_PRF FROM WHS_SAD_INVENTORY_E WHERE IDE_REG_DAT BETWEEN ? AND ? ORDER BY IDE_REG_DAT";
        }
        ...
        try (Connection conn = openConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                ...
                pw.println("  <sheets>");
                pw.println("    <sheet name=\"WHS_SAD_INVENTORY_E\" sheetId=\"1\" r:id=\"rId1\"/>");
                pw.println("  </sheets>");
                pw.println("</workbook>");
                pw.flush();
                zos.closeEntry();
                ...
                for (int colIdx = 0; colIdx < headers.size(); colIdx++) {
                    String colRef = getColName(colIdx + 1) + "1";
                    pw.println("      <c r=\"" + colRef + "\" t=\"inlineStr\"><is><t>" + escapeXml(headers.get(colIdx)) + "</t></is></c>");
                }
                pw.println("    </row>");
                ...
                while (rs.next()) {
                    ...
                    for (int i = 1; i <= columnCount; i++) {
                        String colRef = getColName(i) + rowIdx;
